...
//std::cout<<"__________________________^^^^^^^^^^^^^^^^^^_______________________________"<<std::endl;
  while(stepLengthLeft > 0.0) {
//std::cout<<"__________________________loooooooooooooooooop_______________________________"<<std::endl;
...
//if (theShowino->getDepth()<2) std::cout<<"energyScale[whichCalor]    "<<energyScale[whichCalor]<<std::endl;
...
//      if( hitCalor == Gflash::kNULL) std::cout<<"hitEnergy    "<<hitEnergy/CLHEP::GeV<<std::endl;
...
//std::cout<<"showerType           "<<showerType<<"     whichCalor   "<<whichCalor<<"  depth  "<<showerDepth<<"transDepth    "<<transDepth<<std::endl;
...
//std::cout << "heightProfile " << heightProfile << std::endl;
...
if (kIndex==Gflash::kHB && depth<2) {
//  std::cout<<"c=  "<<longPar[0]<<std::endl;
//  std::cout<<"alpha1= "<<exp(longPar[1])<<std::endl;
//  std::cout<<"beta1= "<<exp(longPar[2])<<std::endl;
//  std::cout<<"alpha2= "<<exp(longPar[3])<<std::endl;
// std::cout<<"beta2= "<<exp(longPar[4])<<std::endl;
//  std::cout<<"X= "<<depth<<std::endl;

//  std::cout<<"Gflash::radLength[kIndex]= "<<Gflash::radLength[kIndex]<<std::endl;
//  std::cout<<"Gflash::intLength[kIndex]= "<<Gflash::intLength[kIndex]<<std::endl;
longPar[0] =0;
}
if (kIndex==Gflash::kHB ) {
longPar[0] =0;
}
...
std::vector<double> GflashHadronShowerProfile::hcalpar(){
  std::vector<double> G(2);
double *myPar;
myPar=longHcal;
    myPar[0] = std::min(1.0,  myPar[0]);
    myPar[0] = std::max(0.0,  myPar[0]);

  if(  myPar[3] > 4.0 ||   myPar[4] > 4.0) {
    double rfactor = 2.0/std::max(  myPar[3],  myPar[4]);
      myPar[3] = rfactor*(myPar[3]+1.0);
      myPar[4] *= rfactor;
}
G[0]=myPar[0];
G[1]=myPar[1];
